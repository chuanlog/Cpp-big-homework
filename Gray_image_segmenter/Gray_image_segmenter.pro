...
SOURCES += \
    Diagonal_Priority_Horizonal_Priority.cpp \
    main.cpp \
    widget.cpp\


HEADERS += \
    Header_Diagonal_Priority_Horizonal_Priority.h \
    Header_Diagonal_Priority_Horizonal_Priority.h \
    segment_functions.h \
    widget.h
...
win32
{
INCLUDEPATH += D:\opencv\OpenCV-CMakeBuild\install\include
LIBS += D:\opencv\OpenCV-CMakeBuild\install\x64\mingw\bin\libopencv_*.dll
# 以上两行在开发时换上自己的路径,这里"libopencv_*.dll"代表bin文件夹下所有以libopencv_开头的ddl文件，找到bin文件夹路径后照抄最后的"libopencv_*.dll"即可
}
# ps:为方便起见，提交至github时应该删去上方路径，可以将自己的路径以注释形式写在下面，便于opencv库的快速导入
...
# D:\opencv\OpenCV-CMakeBuild\install\include
# D:\opencv\OpenCV-CMakeBuild\install\x64\mingw\bin\libopencv_*.dll
# 这是黄俊超的路径
